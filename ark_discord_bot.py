...
if not TOKEN:
    raise ValueError(
        "Bot token is missing! Set DISCORD_BOT_TOKEN in your .env file.")
if not all([VPS_HOST, VPS_USERNAME, VPS_PASSWORD]):
    raise ValueError(
        "VPS credentials are missing! Set VPS_HOST, VPS_USERNAME, and VPS_PASSWORD in your .env file."
    )
...
class SSHClient:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.ssh.connect(hostname=VPS_HOST,
...
async def execute_ark_command(command: str,
                              server_name: str) -> tuple[str, discord.Color]:
...
async def send_response(interaction: discord.Interaction, title: str,
                        output: str, color: discord.Color):
    ...
    if output:
        ...
        if len(output) > 1024:
            ...
            for i, chunk in enumerate(chunks, 1):
                embed.add_field(name=f"Output (Part {i}/{len(chunks)})",
                                value=f"```{chunk}```",
                                inline=False)
        else:
...
async def server_status(interaction: discord.Interaction, server: str):
    ...
    await send_response(interaction, f"🖥️ Server Status: {server}", output,
                        color)
...
async def start_server(interaction: discord.Interaction, server: str):
    ...
    await send_response(interaction, f"🚀 Starting Server: {server}", output,
                        color)
...
async def stop_server(interaction: discord.Interaction, server: str):
    ...
    await send_response(interaction, f"🛑 Stopping Server: {server}", output,
                        color)
...
async def restart_server(interaction: discord.Interaction, server: str):
    ...
    await send_response(interaction, f"🔄 Restarting Server: {server}", output,
                        color)
...
async def list_players(interaction: discord.Interaction, server: str):
    ...
    await send_response(interaction, f"👥 Online Players: {server}", output,
                        color)
...
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
async def broadcast_message(interaction: discord.Interaction, server: str,
                            message: str):
    await interaction.response.defer()
    output, color = await execute_ark_command(f"broadcast {message}", server)
    await send_response(interaction, f"📢 Broadcast Message: {server}", output,
                        color)


@bot.tree.command(name="backup",
                  description="Create a backup of the ARK server")
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
async def backup_server(interaction: discord.Interaction, server: str):
    await interaction.response.defer()
    output, color = await execute_ark_command("backup", server)
    await send_response(interaction, f"💾 Server Backup: {server}", output,
                        color)
...
async def update_server(interaction: discord.Interaction, server: str):
    ...
    await send_response(interaction, f"🔧 Server Update: {server}", output,
                        color)


@bot.tree.command(name="checkupdate",
                  description="Check for game or mod updates")
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
async def check_update(interaction: discord.Interaction, server: str):
    ...
    await send_response(interaction, f"🔍 Check Updates: {server}", output,
                        color)
...
async def check_mod_update(interaction: discord.Interaction, server: str):
    ...
    await send_response(interaction, f"🧩 Check Mod Updates: {server}", output,
                        color)
...
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
async def ban_player(interaction: discord.Interaction, server: str,
                     player_name: str):
    await interaction.response.defer()
    output, color = await execute_ark_command(
        f"rconcmd \"BanPlayer {player_name}\"", server)
    await send_response(interaction, f"🚫 Ban Player: {player_name}", output,
                        color)
...
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
async def kick_player(interaction: discord.Interaction, server: str,
                      player_name: str):
    await interaction.response.defer()
    output, color = await execute_ark_command(
        f"rconcmd \"KickPlayer {player_name}\"", server)
    await send_response(interaction, f"👢 Kick Player: {player_name}", output,
                        color)
...
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
async def unban_player(interaction: discord.Interaction, server: str,
                       player_name: str):
    await interaction.response.defer()
    output, color = await execute_ark_command(
        f"rconcmd \"UnbanPlayer {player_name}\"", server)
    await send_response(interaction, f"✅ Unban Player: {player_name}", output,
                        color)
...
@bot.tree.command(name="destroywilddinos",
                  description="Remove all wild creatures from the map")
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
async def destroy_wild_dinos(interaction: discord.Interaction, server: str):
    await interaction.response.defer()
    output, color = await execute_ark_command("rconcmd \"DestroyWildDinos\"",
                                              server)
    await send_response(interaction, f"🦖 Destroy Wild Dinos: {server}", output,
                        color)


@bot.tree.command(name="settime",
                  description="Set the in-game time (HH:MM format)")
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
async def set_time(interaction: discord.Interaction, server: str, time: str):
    if not re.match(r"^\d{1,2}:\d{2}$", time):
        await interaction.response.send_message(
            "⚠️ Invalid time format. Please use HH:MM format (e.g., 14:30)",
            ephemeral=True)
        return

    await interaction.response.defer()
    output, color = await execute_ark_command(
        f"rconcmd \"SetTimeOfDay {time}\"", server)
    await send_response(interaction, f"🕒 Set Time: {time}", output, color)


@bot.tree.command(name="getchat",
                  description="Get recent chat logs from the server")
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
...
@bot.tree.command(name="getlog",
                  description="Get recent game logs from the server")
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
...
async def show_motd(interaction: discord.Interaction, server: str):
    await interaction.response.defer()
    output, color = await execute_ark_command(
        "rconcmd \"ShowMessageOfTheDay\"", server)
    await send_response(interaction, f"📣 Message of the Day: {server}", output,
                        color)


@bot.tree.command(name="rcon",
                  description="Send a custom RCON command to the server")
@app_commands.choices(
    server=[app_commands.Choice(name=s, value=s) for s in SERVERS])
async def rcon_command(interaction: discord.Interaction, server: str,
                       command: str):
    await interaction.response.defer()
    output, color = await execute_ark_command(f"rconcmd \"{command}\"", server)
    await send_response(interaction, f"🔧 RCON Command: {server}", output,
                        color)


@bot.tree.command(name="help", description="Show available commands")
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(title="🎮 ARK Server Bot Commands",
    ...
    # World Management
    embed.add_field(name="🌍 World Management",
    ...
    embed.add_field(name="ℹ️ Information Commands",
...
async def check_server_status():
    try:
        ...
        # Update bot status
        total_players = sum(info["players"] for info in servers.values()
                            if info["running"])
